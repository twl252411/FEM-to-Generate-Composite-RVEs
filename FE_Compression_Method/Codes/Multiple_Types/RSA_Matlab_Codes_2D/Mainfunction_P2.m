...
Radius  = max(EqvRadius)*1.05; % Inclusion length, radius, volume fraction
Tvalue = 200.0^2*0.65/(sum(AreaMat)); 
NumI = zeros(1,9);
for i = 1:9
    NumI(1,i) = floor(Tvalue);
    if i <= ceil((Tvalue - floor(Tvalue))*9)
        NumI(1,i) = NumI(1,i) + 1;
    end
end
Area = dot(NumI,AreaMat);
NumInc = sum(NumI);
NumP1 = randi([6,7]);
NumP2 = randi([6,7]);
NumInc2 = [NumP1, NumP2];
NumInc1 = NumInc - sum(NumInc2) - 1;
%
%--------------------------------------------------------------------------------------------------------------------------------------------------------------- 
%
...
while Iter >= 1
    ...
    %
    LastA = Last4oritensor(Inclusion.orientation,AreaMat,Area,NumInc);
    TempA = PredA - LastA;
    %
    Funcx = 0;
    for i = 1:2
...
LastA = Last2oritensor(Inclusion.orientation,AreaMat,Area,NumInc);
%
%--------------------------------------------------------------------------------------------------------------------------------------------------------------- 
%
Geometry.Rvesize = [460, 460] - Radius*2.0*ones(1,2);
Iteration = 1;
Inclusion.midpoint = [];
...
%--------------------------------------------------------------------------------------------------------------------------------------------------------------- 
%
Rvesize = 200 - 2*Radius; 
L = 100 + Radius*1.25;
Inclusion.midpoint2 = zeros(2*sum(NumInc2), 2);
%  
for iii = 1:2
    Iteration = 1;
    midpoint = [];
