...
for ifl in range(Filenum):
    ...
    RLength, NumEle1, NumEle2, Ampf = 200.0, int(50*1.0), 50, 1.0
    f1 = 'C:\\Temp\\Generated_Points_Angles\\' + strname + '\\NewCentroids' + str(FilId) + '.txt'
    f2 = 'C:\\Temp\\Generated_Points_Angles\\' + strname + '\\NewOriangles' + str(FilId) + '.txt'
    #
    UserVariables = {}
    ...
    #-------------------------------------------------------------------------------------------------------
    #
    f = open('C:\\Temp\\Statistical_Analysis\\Two_Points_Probability\\' + strname + '_RVE\\VoxelElements' + str(FilId) + '.txt', 'w')
    for x, y in itertools.product(range(NumEle1), range(NumEle2)):
        if y < NumEle2 - 1:
